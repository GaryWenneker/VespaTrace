# PRP File Generator for VS Code
# Usage: .\Generate-PRP.ps1 -FeatureName "User Authentication" -OutputDir "requirements"
...
$SafeFileName = $FeatureName -replace '[^a-zA-Z0-9\s]', '' -replace '\s+', '_'
$FileName = "PRP_$SafeFileName.md"
$FilePath = Join-Path $OutputDir $FileName

# Generate timestamp
$Timestamp = Get-Date -Format "MMMM d, yyyy"

# PRP Template Content
$PRPContent = @"
# Product Requirement Prompt: $FeatureName
...
# Write the PRP file
try {
    $PRPContent | Out-File -FilePath $FilePath -Encoding UTF8
    Write-Host "PRP file generated successfully!" -ForegroundColor Green
    Write-Host "File location: $FilePath" -ForegroundColor Cyan
    Write-Host "Open in VS Code: code `"$FilePath`"" -ForegroundColor Yellow
    
    # Optionally open in VS Code
    $OpenInVSCode = Read-Host "Open in VS Code now? (y/N)"
    if ($OpenInVSCode -eq 'y' -or $OpenInVSCode -eq 'Y') {
        code $FilePath
    }
}
catch {
    Write-Host "Error generating PRP file: $_" -ForegroundColor Red
}