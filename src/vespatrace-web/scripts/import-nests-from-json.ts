...
async function main() {
  ...
  for (const o of obs) {
    ...
    if (!lat || !lon) continue
    // We treat each observation as a potential nest report if it indicates nest in meta, else still ingest as point-of-interest
    const found_at = o.dateTime ? new Date(o.dateTime.replace(' ', 'T')) : (o.date ? new Date(o.date) : new Date())
    ...
    try {
      ...
      console.warn('Skip import for', external_id, (e as Error).message)
    }
  }
